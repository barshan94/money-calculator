...
async function cleanupAsset(
   ...
   *
   * Therefore the asset row must be deleted
   * before the purchase transaction.
   */
  const {
    data: transactions,
   ...
   * lookup did not find the purchase transaction.
   */
  ...
  () => {
    ...
    it(
      ...
      async () => {
        ...
        try {
          /*
           * This is test fixture setup, not the
           * operation under test. Use the admin
           * client so the test remains valid after
           * authenticated direct UPDATE is revoked.
           */
          const {
            error: statusError,
          } = await admin
