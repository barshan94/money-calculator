...
async function createWhatIfHistory(page: any) {
  ...
  await expect(page.getByText(incomeCategory, { exact: true })).toBeVisible({ timeout: 15000 });

  await page.getByLabel("Category name").fill(expenseCategory);
  await page.getByLabel("Type").selectOption("expense");
  await page.getByRole("button", { name: "Create Category", exact: true }).click();
  await expect(page.getByText(expenseCategory, { exact: true })).toBeVisible({ timeout: 15000 });
  ...
  const incomeCategorySelect = page.locator("#income-category");
  await expect(incomeCategorySelect).toBeVisible();
  await expect(
    incomeCategorySelect.locator("option", { hasText: incomeCategory }),
  ).toHaveCount(1);
  await incomeCategorySelect.selectOption({ label: incomeCategory });

  const incomeAccountSelect = page.locator("#income-account");
  await expect(incomeAccountSelect).toBeVisible();
  await incomeAccountSelect.selectOption({ index: 1 });

  await page.getByLabel("Description").fill(`E2E What If Income ${suffix}`);
  ...
  const expenseCategorySelect = page.locator("#expense-category");
  await expect(expenseCategorySelect).toBeVisible();
  await expect(
    expenseCategorySelect.locator("option", { hasText: expenseCategory }),
  ).toHaveCount(1);
  await expenseCategorySelect.selectOption({ label: expenseCategory });

  const expenseAccountSelect = page.locator("#expense-account");
  await expect(expenseAccountSelect).toBeVisible();
  await expenseAccountSelect.selectOption({ index: 1 });

  await page.getByLabel("Description").fill(`E2E What If Expense ${suffix}`);
  await page.getByRole("button", { name: /save|create|add|submit/i }).click();
